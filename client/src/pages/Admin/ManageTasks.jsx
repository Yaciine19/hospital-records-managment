...
function ManageTasks() {
  ...
  const getAllTasks = async () => {
    try {
      const response = await axoisInstance.get(API_PATHS.TASKS.GET_ALL_TASKS, {
        params: {
  ...
  const handleDownloadReport = async () => {
    try {
      const response = await axoisInstance.get(API_PATHS.REPORT.EXPORT_TASKS, {
        responseType: "blob",
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          {allTasks?.map((item) => (
            <TaskCard
              ...
              completedTodoCount={item.completedTasksCount || 0}
